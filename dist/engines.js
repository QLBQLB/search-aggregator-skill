 ...
 */
export async function parallelSearch(tasks) {
    const results = new Map();
    const settled = await Promise.allSettled(Array.from(tasks.entries()).map(([name, promise]) => promise.then(result => ({ name, result }))));
    for (const outcome of settled) {
        if (outcome.status === 'fulfilled') {
            ...
            // 失败时记录错误
            results.set(Array.from(tasks.keys()).find(k => outcome.reason?.message?.includes(k)) || 'unknown', outcome.reason);
        }
    }
    return results;
}
 ...
 * 这里提供解析工具函数
 */
export function parseSearchResults(source, rawData) {
    const results = [];
    try {
        // 根据不同源解析不同格式
        switch (source) {
