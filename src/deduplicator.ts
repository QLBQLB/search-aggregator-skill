 ...
 * 判断两个 URL 是否相似
 */
export function areUrlsSimilar(url1: string, url2: string, threshold = 0.8): boolean {
...
export function deduplicateResults(
  ...
  const normalized = new Map<string, SearchResult>();
...
): { results: SearchResult[]; stats: { original: number; deduped: number; rate: number } } {
